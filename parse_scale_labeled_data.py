...
def extract_frame(vidcap, start, end, label, task_id, tsv, freq = 25):
...
URL = 'https://storage.yandexcloud.net/cvproject/labeled_scale.json'
r = requests.get(URL)
data = r.json()
...
for video in data:
    ...
    label = "Void"
    classes = set()
    with open(os.path.join(CLASSIFICATION_DIR,'_classes.csv'),'w') as tsv:
        for event in events:
